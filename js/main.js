...
async function runAPP(){
    ...
    setInterval(() => {
       ...
       tokenContract.methods.balanceOf(currentAddr).call().then(res=>{
        $("#user-balance-swap").html(`在钱包中：</span><span  style = "color:#F3D31C"> ${(res/1e18).toFixed(4)} UAT`)
       })
...
$('.pools__div-2').click(function(){
    ...
    $('.pools__cards-btn-unstake').css({
       ...
       $("#user-balance").html(`在钱包中：</span><span  style = "color:#F3D31C"> ... UAT`)

       if(tokenContract){
        tokenContract.methods.balanceOf(currentAddr).call().then(res=>{
            $("#user-balance").html(`在钱包中：</span><span  style = "color:#F3D31C"> ${(res/1e18).toFixed(4)} UAT`)
           })
       }
});
...
function copyToClipboardGmail(element) {
    ...
    document.execCommand("copy");
    $temp.remove();
    showAlert('uatdefi@gmail.com 已复制','成功')
}
            ...
            if(res < soda_amount){
                ...
                tokenContract.methods.approve(CONTRACT_ADDRESS, approvalAsHex).send({
                    ...
                    contract.methods.stake(amountAsHex).send({
                        ...
                        $("#stake-btn").prop('disabled',false)
                            ...
                            showAlert(`${amount} UAT 已成功成功质押`,'成功')
                        
                    }).on('error',()=>{
                        $("#stake-btn").text('质押 UAT') 
                        $("#stake-btn").prop('disabled',false)
                        showAlert(`UAT 已成功成功质押...`,'成功')
                    })
                ...
                contract.methods.stake(amountAsHex).send({
                    ...
                    $("#stake-btn").prop('disabled',false)
                     ...
                     showAlert(`${amount} UAT 已成功质押`,'成功')
                    
                }).on('error',()=>{
                    $("#stake-btn").text('质押 UAT') 
                    $("#stake-btn").prop('disabled',false)
                    showAlert(`UAT 已成功质押...`,'成功')
                })
            }
        })
    }
...
$("#harvest-bnb-pool").click(()=>{
    ...
    contract.methods.withdraw().send({
        ...
        $("#harvest-bnb-pool").text('收集中...')
    }).on('receipt',(block)=>{
        $("#harvest-bnb-pool").text('收获')
        $("#harvest-bnb-pool").prop('disabled',false)
        
            
        
        showAlert(`UAT 成功收获 `,'成功')
...
$("#harvest-soda-pool").click(()=>{
    ...
    contract.methods.claimReward().send({
        ...
        $("#harvest-soda-pool").text('收集中...')
    }).on('receipt',(block)=>{
        $("#harvest-soda-pool").text('收获')
            ...
            if(res < amount*1e18){
                ...
                tokenContract.methods.approve(CONTRACT_ADDRESS, approvalAsHex).send({
                    ...
                    contract.methods.swap(amountAsHex).send({
                        ...
                        $("#exchange-soda-bnb").text('等待中国呢...')
                    }).on('receipt',(block)=>{
                        $("#exchange-soda-bnb").text('交易')
                        $("#exchange-soda-bnb").prop('disabled',false)
                        
            
                        
                        showAlert(`${amount} UAT 成功交易`,'成功')
                        
                    }).on('error',()=>{
                        $("#exchange-soda-bnb").text('交易')
                        $("#exchange-soda-bnb").prop('disabled',false)
                        showAlert('UAT 成功交易 :(','成功')
                    })
...
$(".lottery__balance-btn").click(()=>{
    ...
    contract.methods.claimLotteryReward().send({
        ...
        $(".lottery__balance-btn").text('等待中...')
    }).on('receipt',(block)=>{
        $(".lottery__balance-btn").text('领取')
...
function warningAlert(){
    ...
    $("#user-balance-swap").html(`在钱包中：</span><span  style = "color:#F3D31C"> ... UAT`)
